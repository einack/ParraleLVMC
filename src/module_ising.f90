...
MODULE functions
	...
	SUBROUTINE initialize(a)
		...
		! Parameters initialization
	   	...
	   	READ(17,*)
	   	READ(17,*) Jo,hfield,hlong,nstep1,nstep2,nwalk,mu,&
				  Lx,seed1 ,beta_r,beta_s,Jrs
	   	CLOSE(17)
...
!********************************************************************************************
  ...
  subroutine vmc(beta_r,beta_s,Jrs,energy,energy_err,derivative)
   ...
   DO m = 1, nstep1
      ...
      IF ((MOD(m,ncorr0)==0) .AND. (m.GT.ncorr0))THEN
         eebavg  = eebavg  + var_E 
...
!    Wavg     = wa(2)/wa(4)
    ...
    WRITE(10,'(F12.3,3F15.8)') hfield/Jo, beta,  Eave, sigmaE
    WRITE(13,'(F7.2,4F15.8)') hfield/Jo, beta, Mave, sigmaM
